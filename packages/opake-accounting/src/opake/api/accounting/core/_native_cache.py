...
Contributor-internal: not re-exported from the ``opake.accounting`` façade.
"""
...
#: opake-dpftrl lives at module scope so it stays alive for the lifetime
...
class NativeCache:
    ...
    def _refresh_max_bytes(self) -> None:
        """Re-read the env var so a runtime change to ``OPAKE_*_MAX_BYTES``
    ...
    def get_or_create(self, key: tuple, factory: Callable[[], int]) -> int | None:
        ...
        .. warning::
            ...
            alongside :func:`opake.accounting.calibration.calibrate`.
        """
    ...
    def with_handle(
        ...
        :func:`opake.accounting.calibration.calibrate`'s ``finally`` clause)
