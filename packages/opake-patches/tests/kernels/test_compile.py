"""``torch.compile`` over the opake-patches Triton kernels.
...
Triton kernels typically appear as opake ops to Dynamo and may produce
...
CUDA-only — Triton kernels in opake-patches don't have a CPU path.
"""
...
from opake.patches.kernels import opake_rms_norm, opake_swiglu

pytestmark = [
    pytest.mark.cuda,
    pytest.mark.skipif(
        not torch.cuda.is_available(), reason="opake-patches kernels require CUDA"
    ),
]


class _RmsNormBlock(nn.Module):
    """Tiny module exercising ``opake_rms_norm`` + a SwiGLU MLP."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = opake_rms_norm(x, self.norm_weight)
        gate = self.gate(x)
        up = self.up(x)
...
def test_kernels_under_compile_cuda(backend: str):
    """opake-patches Triton kernels survive torch.compile on CUDA."""
    model, x, y = _build()
    eager = _run(model, x, y, compile_backend=None)
