...
from opake.api.patches.kernels import _grouped_moe, _moe_memory
from opake.api.patches.kernels import moe as moe_kernel
from opake.api.patches.kernels._grouped_moe import Opake_GroupedMoE
from opake.api.patches.kernels.moe import (
    Opake_MoE,
    _expert_route,
    torch_reference_moe,
)
...
def test_opake_moe_dispatch_respects_workspace_budget(monkeypatch):
    ...
    monkeypatch.setattr(
        _grouped_moe.Opake_GroupedMoE,
        "apply",
        staticmethod(lambda *args: sentinel),
    )
    from opake.api.patches.kernels import _moe_dispatch
    ...
    dense = moe_kernel.opake_moe(x, gate_up, down, index, weights)
    ...
    assert moe_kernel.opake_moe(x, gate_up, down, index, weights) is sentinel
    ...
    dense_result = moe_kernel.opake_moe(
...
def test_dense_and_grouped_compute_only_requested_expert_gradient():
    ...
    for op in (Opake_MoE, Opake_GroupedMoE):
        loss = make_loss(op)
        for argnum in (1, 2):
...
def test_dense_output_allows_inplace_consumer():
    ...
    down.requires_grad_()
    out = Opake_MoE.apply(x, gate_up, down, index, weights)
...
def test_dense_backward_flattens_multiple_token_dimensions():
    ...
    Opake_MoE.apply(x, gate_up, down, index, weights).square().mean().backward()
...
def test_forced_forward_chunks_bound_dense_linear_rows(monkeypatch):
    ...
    expected = Opake_MoE.apply(x2, gate_up, down, index2, weights2)
    ...
    actual = Opake_MoE.apply(x2, gate_up, down, index2, weights2)
...
def test_forced_dense_backward_matches_autograd_reference(monkeypatch):
    ...
    actual = vmap(
        grad(
            lambda xx, g, d, ii, ww: loss(Opake_MoE.apply, xx, g, d, ii, ww),
            argnums=(0, 1, 2, 4),
        ),
        in_dims=in_dims,
    )(x, gate_up, down, index, weights)
...
def test_forced_forward_chunks_bound_grouped_mm_rows(monkeypatch):
    ...
    actual = Opake_GroupedMoE.apply(x2, gate_up, down, index2, weights2)
    expected = Opake_MoE.apply(x2, gate_up, down, index2, weights2)
    torch.testing.assert_close(actual, expected, rtol=1e-3, atol=1e-3)
    assert len(rows) > 2
...
def test_forced_weight_grad_tiles_share_route_plan(monkeypatch):
    ...
    def loss(xx, g, d, ii, ww):
        return Opake_GroupedMoE.apply(xx, g, d, ii, ww).square().mean()
...
def test_forced_trainable_backward_chunks_single_long_example(monkeypatch):
    ...
    def dense_loss(xx, g, d, ii, ww):
        return Opake_MoE.apply(xx, g, d, ii, ww).square().mean()
...
def test_forced_bf16_chunks_accumulate_weight_gradients_in_fp32(monkeypatch):
    ...
    def grouped_loss(xx, g, d, ii, ww):
        return Opake_GroupedMoE.apply(xx, g, d, ii, ww).float().square().mean()

    def dense_loss(xx, g, d, ii, ww):
        return Opake_MoE.apply(xx, g, d, ii, ww).float().square().mean()
