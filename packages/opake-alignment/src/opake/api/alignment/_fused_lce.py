...
"""Bridge to the opake-patches fused linear cross-entropy kernel.

The fused alignment primitives — :func:`~opake.api.alignment.sft.loss.fused_nll_loss`,
:func:`~opake.api.alignment.sft.loss.fused_dft_loss`, and
:func:`~opake.api.alignment.logprob.fused_sequence_logp` — share one accelerated
backend: the patches Triton ``Opake_LinearCrossEntropyLoss`` kernel. It computes
...
``opake-patches`` is the optional ``opake-alignment[patches]`` dependency, so
...
# Patches is the optional ``opake-alignment[patches]`` extra, not a runtime
# dependency. Resolve the kernel module dynamically so the bridge remains
# optional.
_LCE_KERNEL_PATH = "opake.api.patches.kernels.linear_cross_entropy"
_CE_KERNEL_PATH = "opake.api.patches.kernels.cross_entropy"
# Pure-PyTorch chunked kernel (Triton-free): the fused-CE path on MPS/CPU.
_LCE_CHUNKED_PATH = "opake.api.patches.kernels._linear_ce_chunked"
...
def lce_available(hidden: torch.Tensor) -> bool:
    ...
    works in any float dtype. ``opake-patches`` must be importable either way
...
def selective_log_softmax_available(logits: torch.Tensor) -> bool:
    ...
    Needs CUDA and ``opake-patches`` importable. The kernel casts inputs to fp32
...
def import_ce_kernel():
    ...
    static AST clean of ``import opake.api.patches.*`` so the
    ...
    *,
    ...
    if hidden.is_cuda:
        ...
        nll_sum, _lse, _valids, _token_weight = (
            kernel_mod.Opake_LinearCrossEntropyLoss.apply(
