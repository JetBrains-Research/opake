...
from opake.api.accounting.dpftrl.amplification._balls_in_bins import BallsInBins
...
def _reset_caches(monkeypatch):
    monkeypatch.delenv("OPAKE_BNB_TRANSCRIPT_CACHE_MAX_BYTES", raising=False)
    monkeypatch.setattr(
...
def test_oversized_corpus_preserves_one_shot_fallback(monkeypatch):
    monkeypatch.setenv("OPAKE_BNB_TRANSCRIPT_CACHE_MAX_BYTES", "0")
    calls = 0
    original_one_shot = _native.bnb_mc_pld
