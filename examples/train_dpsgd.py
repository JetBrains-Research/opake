...
def parse_args():
    ...
    dp_group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Release the projected normalized aggregate and its clean "
        "element-wise square with DP-SGD JME. Requires plain Gaussian noise, "
        "plain independent Poisson sampling, scalar clipping, "
        "--jme-aggregate-norm, and --jme-allocation.",
    )
    dp_group.add_argument(
        "--jme-aggregate-norm",
        type=float,
    ...
    dp_group.add_argument(
        ...
        type=float,
        default=None,
        help="Required with --jme-allocation first_variance_cap; caps first-stream "
        "variance relative to releasing only the projected aggregate.",
    )
    dp_group.add_argument(
        "--per-group-clipping",
        type=str,
...
def main():
    ...
    if use_jme:
        ...
        if truncated_batch_size is not None or use_parallel_poisson or use_horizon_sampler:
            raise ValueError(
                "--jme currently supports only plain independent Poisson sampling; "
                "truncated, parallel, and horizon-allocation samplers require "
                "separate joint-sensitivity derivations."
            )
    ...
    # Projected JME whitens to a sensitivity-one Gaussian query, so the
    # ordinary Gaussian mechanism is a conservative dominating PLD.
