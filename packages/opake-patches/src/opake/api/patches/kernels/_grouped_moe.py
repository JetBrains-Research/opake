...
same Function/vmap contract as the dense :class:`~opake.api.patches.kernels.moe.Opake_MoE`
baseline, but built on ``torch._grouped_mm`` instead of Triton so it runs on
Apple MPS (and CPU). ``opake_moe`` dispatches here on non-CUDA hosts when
...
class Opake_GroupedMoE(torch.autograd.Function):
    """Sparse grouped-GEMM MoE expert FFN (non-Triton). Same ``.apply`` signature
    as the dense ``Opake_MoE`` and the Triton ``Opake_FusedMoE``."""
