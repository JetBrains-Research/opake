...
Registers the single-stream :class:`opake.dpftrl.noise._engine.MFNoiseState`
and the paired
:class:`opake.dpftrl.noise._second_moment.SecondMomentMFNoiseState` sync
handlers with :func:`opake.distributed.sync` at import time.  Imported for
its side effects from :mod:`opake.dpftrl.noise`; not re-exported.
"""
...
def _assert_rng_key_equal(state: MFNoiseState, state_name: str) -> None:
    ...
    sensitivity fingerprints. A seed is opake identity material rather than a
