...
from opake.api.patches.kernels._utils import cast_to_dtype
...
def _lora_case(variant: str, trainable_adapters: bool):
    ...
    if variant == "w":
        ...
        return opake_lora_w, args, args[1:4]
    ...
    if variant == "qkv":
        ...
        return opake_lora_qkv, args, weights
    ...
    args = (x, *gate, *up, *down)
    weights = tuple(arg for arg in args[1:] if isinstance(arg, torch.Tensor))
    return opake_lora_mlp, args, weights
...
def test_lora_autocast_vmap_grad_with_fp32_weights(amp_dtype):
    ...
    def loss(xi, a, b):
        with torch.autocast("cuda", dtype=amp_dtype):
            return opake_lora_w(xi, W, a, b, 0.2).float().square().mean()
...
def test_fused_moe_fp32_weights_match_full_cast(trainable_experts, amp_dtype):
    ...
    with torch.autocast("cuda", dtype=amp_dtype):
        outputs, loss = _outputs_and_loss(opake_fused_moe, args)
    grads = _input_grads(loss, args)
    cast_args = _clone_args(args, amp_dtype)
    cast_outputs, cast_loss = _outputs_and_loss(opake_fused_moe, cast_args)
    cast_grads = _input_grads(cast_loss, cast_args)
...
def test_fused_moe_autocast_vmap_grad_with_fp32_weights(amp_dtype):
    ...
    def loss(xi, index, weight):
        with torch.autocast("cuda", dtype=amp_dtype):
            return opake_moe(xi, gate_up, down, index, weight).float().square().mean()
...
def test_lora_autocast_cast_peak_is_bounded_per_layer(trainable_adapters):
    ...
    def forward():
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            for W, A, B in layers:
                output = opake_lora_w(output, W, A, B, 0.2)
        return output
    ...
    def forward():
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            for gate_up, down in layers:
                output = opake_moe(output, gate_up, down, top_k_index, top_k_weights)
        return output
