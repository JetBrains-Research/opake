...
from opake.api.patches.kernels import lora as lora_kernels
from opake.api.patches.kernels.lora import (
    ACTIVATION_SWIGLU,
    Opake_LoRA_MLP,
    Opake_LoRA_QKV,
    Opake_LoRA_W,
...
def opake_lora_linear(X, W, A, B, scaling):
    """Opake kernel implementation."""
    return Opake_LoRA_W.apply(X, W, A, B, scaling)
...
def opake_lora_mlp(X, Wg, Ag, Bg, Sg, Wu, Au, Bu, Su, Wd, Ad, Bd, Sd):
    """Opake kernel implementation."""
...
@pytest.mark.parametrize(
    ("vmap_rule", "in_dims"),
    [
        (Opake_LoRA_W.vmap, (0, 0, None, None, None)),
...
def test_rectangular_linear_and_qkv_vmap_gradients_match_pytorch(assert_precision):
    ...
    assert_precision(
        opake_lora_linear(X[0], Wq, Aq, Bq, SCALING),
    ...
    def linear_op(x):
        return opake_lora_linear(x, Wq, Aq, Bq, SCALING).square().mean()
    ...
    def qkv_op(x):
        return sum(
            projection.square().mean()
            for projection in opake_lora_qkv(
                x, Wq, Aq, Bq, SCALING, Wk, Ak, Bk, SCALING, Wv, Av, Bv, SCALING
            )
        )
...
class TestLoRAWForward:
    ...
    def test_forward_matches_pytorch(self, assert_precision, mellum_config):
        ...
        out_op = opake_lora_linear(X, W, A, B, SCALING)
...
class TestLoRAWBackward:
    ...
    def test_backward_matches_pytorch(self, assert_precision, mellum_config):
        ...
        out_op = opake_lora_linear(X_op, W, A_op, B_op, SCALING)
    ...
    def test_incomplete_adapter_matches_base_projection(self, missing):
        ...
        out = opake_lora_linear(X, W, A, B, SCALING)
        ...
        got = vmap(grad(lambda x: opake_lora_linear(x, W, A, B, SCALING).sum()))(X_vmap)
...
class TestLoRAWVmapForward:
    ...
    def test_vmap_forward_precision(self, assert_precision, mellum_config):
        ...
        out_op = vmap(lambda x: opake_lora_linear(x, W, A, B, SCALING))(X)
...
class TestLoRAWVmapGrad:
    ...
    def test_vmap_grad_precision(self, assert_precision, mellum_config):
        ...
        def f_op(x):
            return opake_lora_linear(x, W, A, B, SCALING).mean()
...
class TestLoRAWPerformance:
    ...
    def test_forward_performance(
        ...
        """Forward performance: opake vs pytorch."""
        BATCH = mellum_config["batch_size"]
        SEQ = mellum_config["seq_len"]
        HIDDEN = mellum_config["hidden_dim"]
        RANK = mellum_config["rank"]
        ...
        def opake_fn(x):
            return opake_lora_linear(x, W, A, B, SCALING)

        pt_stats = measure_time_and_memory(pytorch_fn, X)
        op_stats = measure_time_and_memory(opake_fn, X)
    ...
    def test_backward_performance(
        ...
        """Backward performance: opake vs pytorch."""
        BATCH = mellum_config["batch_size"]
        SEQ = mellum_config["seq_len"]
        HIDDEN = mellum_config["hidden_dim"]
        RANK = mellum_config["rank"]
        ...
        op_stats = measure_time_and_memory(opake_fn, X, A, B)
...
class TestLoRAQKVPerformance:
    ...
    def test_backward_performance(
        ...
        def opake_fn(x, aq, bq, ak, bk, av, bv):
            Q, K, V = opake_lora_qkv(
                x, Wq, aq, bq, SCALING, Wk, ak, bk, SCALING, Wv, av, bv, SCALING
            )
            return Q + K + V
        ...
        op_stats = measure_time_and_memory(opake_fn, X, Aq, Bq, Ak, Bk, Av, Bv)
...
class TestLoRAMLPPerformance:
    ...
    def test_backward_performance(
        ...
        def opake_fn(x, ag, bg, au, bu, ad, bd):
            return opake_lora_mlp(
                x, Wg, ag, bg, SCALING, Wu, au, bu, SCALING, Wd, ad, bd, SCALING
            )

        pt_stats = measure_time_and_memory(pytorch_fn, X, Ag, Bg, Au, Bu, Ad, Bd)
        op_stats = measure_time_and_memory(opake_fn, X, Ag, Bg, Au, Bu, Ad, Bd)
...
def _staged_backward_loss(kind, *, trainable=True, tokens=8, hidden=32, rank=4):
    ...
    if kind == "w":
        ...
        loss = opake_lora_linear(X, W, A, B, SCALING).sum()
    elif kind == "qkv":
        projections = []
        for _ in range(3):
            W = _kaiming_weight(hidden, hidden, **kw)
        ...
        loss = sum(Opake_LoRA_QKV.apply(X, *projections)).sum()
    else:
...
class TestLoRAMLPSecondOrder:
    ...
    def test_create_graph_raises(self):
        ...
        (grad_X,) = torch.autograd.grad(
            opake_lora_mlp(*args).sum(), args[0], create_graph=True
        )
    ...
    def test_jacrev_jacrev_raises(self):
        ...
        def fn(x):
            return opake_lora_mlp(x, *rest).sum()
