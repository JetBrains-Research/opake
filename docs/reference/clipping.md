...
6. **[`moe_clipped_grad()`](../mechanisms/dp-sgd/moe-load-balancing.md)** — Per-example clipping for mixture-of-experts models with the Switch load-balancing loss; releases the batch router load inside the clipper and carries the next step's load estimate in its state. Accounted by `moe_aux`.
...
- **`MoeClipState`** — State of `moe_clipped_grad`: the public load estimate `f_tilde`, the filter, the release stream and its public constants; exposes `step`, `load_noise_std`, `filtered_noise_std` and the `imbalance` monitor.
