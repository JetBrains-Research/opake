...
from opake.api.patches.transformers._registry import detect_family, get_family_apply_fn
...
def _has_kernel_runtime() -> bool:
    ...
    Delegates to :func:`opake.api.engine.device.fused_kernels_available` so the
...
def _patch_forward(
    ...
    if hasattr(target_cls, "forward") and not hasattr(
        target_cls.forward, "__opake_patched__"
    ):
    ...
    if model is not None:
        for module in model.modules():
            ...
            if type(module) is target_cls and not hasattr(fwd_fn, "__opake_patched__"):
...
def apply_transformers_model_patches(
    ...
    **kwargs,
    ...
    logger.debug("opake: Applied model patches for %s", family)
    ...
    if dropout and model is not None:
        from opake.api.patches.transformers.components.dropout import disable_dropout
