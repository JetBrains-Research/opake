...
from opake.api.patches.kernels._moe_dispatch import moe_dispatch_decision
from opake.api.patches.kernels._moe_memory import estimate_moe_workspace
...
def main() -> None:
    ...
    implementations = {"dense": Opake_MoE.apply}
    if grouped_mm_available() and args.device in ("cpu", "mps"):
        implementations["grouped"] = Opake_GroupedMoE.apply
    if (
        triton_available
        and args.device == "cuda"
        and decision.features.dtype in (torch.float16, torch.bfloat16)
    ):
        from opake.api.patches.kernels.fused_moe import Opake_FusedMoE

        implementations["triton"] = Opake_FusedMoE.apply

    with autocast():
        reference = Opake_MoE.apply(x, gate, down, index, weights).detach()
    rows = []
