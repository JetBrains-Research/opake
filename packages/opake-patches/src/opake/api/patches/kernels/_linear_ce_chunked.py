...
``Opake_LinearCrossEntropyLoss``. It never materializes the full
...
def linear_nll_sum_chunked(
    ...
    Takes the same positional args as ``Opake_LinearCrossEntropyLoss.apply``
...
def linear_cross_entropy_chunked(
    ...
    """Mean-reduced chunked linear CE — matches ``opake_linear_cross_entropy_loss``.
