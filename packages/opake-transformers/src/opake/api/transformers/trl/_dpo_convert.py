"""trl.DPOConfig → opake DPOConfig manifest + converter.
...
from opake.api.transformers.trainer._convert import _normalize_dp_overrides
...
# Opake's implemented DPO heads (opake's own naming convention).
_OPAKE_DPO_LOSS_TYPES = frozenset(
    {
        "sigmoid",
        "hinge",
        "ipo",
...
# TRL head name → opake head name, where opake pursues its own (clearer)
# convention. TRL's ``"sft"`` is the chosen-completion NLL regulariser, which
# opake calls ``"chosen_nll"``. The converter translates on the way in.
_TRL_TO_OPAKE_LOSS_TYPE = {"sft": "chosen_nll"}


def _loss_type_transform(trl: dict[str, Any]) -> dict[str, Any]:
    """Translate + validate every entry in ``loss_type`` against opake's heads.

    TRL's ``"sft"`` head is renamed to opake's ``"chosen_nll"`` (see
    ``_TRL_TO_OPAKE_LOSS_TYPE``). Adversarial Optimal Transport heads (``aot``,
    ``aot_unpaired``), which opake does not implement, are rejected.
    """
    ...
    mapped = [_TRL_TO_OPAKE_LOSS_TYPE.get(v, v) for v in values]
...
TRL_DPO_REJECTED_FIELDS: dict[str, Callable[[Any], str | None]] = {
    ...
    # ``precompute_ref_log_probs`` is not rejected here: opake always
...
TRL_DPO_DROP_FIELDS: dict[str, str | Callable[[Any], str | None]] = {
    "precompute_ref_log_probs": (
        "Opake always precomputes reference logps under DP for static-"
...
def _convert_trl_dpo_config(
    ...
    **dp_overrides: Any,
    ...
    """Translate a ``trl.DPOConfig`` instance into opake ``DPOConfig`` kwargs."""
    trl = _import_trl()
