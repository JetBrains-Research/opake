...
(:func:`opake.api.accounting.core._process_codec._load_dp_process`)
looks types up by name from that registry.  If the composition module
imports :class:`Composed` / :class:`Repeated` lazily, plain
``import opake.accounting`` leaves the registry incomplete and
round-tripping a tree containing those nodes fails with
``ValueError: Unknown nested DpProcess type 'Composed' in field 'inner'``.

Tests use only opake-accounting-owned process types (EpsDelta,
Composed, Repeated, CachedProcess) so they don't require opake-dpsgd
or opake-dpftrl.
"""
...
def test_cached_composed_repeated_roundtrips_via_json() -> None:
    ...
    checkpoint: a cached opake-boundary at the top, a Composed node
