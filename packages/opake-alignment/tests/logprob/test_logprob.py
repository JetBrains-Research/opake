...
(mirrors ``opake-engine/tests/functional/test_functional.py``). Imports target
...
from opake.api.alignment.logprob._sequence import (
    fused_sequence_logp,
    sequence_logp,
)
...
def test_sequence_logp_vmap_grad_finite() -> None:
    ...
    opake-engine's functional tests. grad is taken over the float ``logits``
...
def test_selective_log_softmax_available_false_on_cpu() -> None:
    ...
    from opake.api.alignment._fused_lce import selective_log_softmax_available
