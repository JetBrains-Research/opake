...
def _pytorch_causal_lm_loss(
    ...
    **kwargs,
    ...
    signature documents that opake honors it.  HF's stock
    ``ForCausalLMLoss`` accepts it via ``**kwargs`` and silently drops
    it; HF Trainer rebuilds the loss separately.  Opake honors it
    inline.
    """
    logits = logits.float()
...
def _opake_causal_lm_loss(
    ...
    **kwargs,
    ...
    """CausalLM loss using Opake cross-entropy Triton kernel.
...
def apply_causal_lm_loss_function_patch(model, target_cls: type[nn.Module]) -> bool:
    """Attach Opake's causal-LM loss to matching model instances."""
    if model is None:
        return False
    ...
    for module in model.modules():
        if type(module) is target_cls:
            module.loss_function = _opake_causal_lm_loss
            patched = True
    return patched
        ...
        **kwargs,
        ...
        if use_fused_ce:
            ...
            if not use_chunked_ce:
                ...
                ce_loss_fn = Opake_LinearCrossEntropyLoss.apply
            else:
                from opake.api.patches.kernels._linear_ce_chunked import (
