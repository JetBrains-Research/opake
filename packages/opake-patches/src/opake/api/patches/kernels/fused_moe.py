...
"""Fused grouped-GEMM MoE expert FFN — sparse (O(T*K)) perf path for ``opake_moe``.

A re-derivation of Liger's ``LigerFusedMoEFunction`` made vmap- and
vmap(grad)-safe for Opake's per-sample-gradient DP-SGD path. Same Function/vmap
contract as the dense :class:`~opake.api.patches.kernels.moe.Opake_MoE`
baseline, so ``opake_moe`` dispatches CUDA+bf16/fp16 here and keeps the dense
...
class Opake_FusedMoE(torch.autograd.Function):
    """Sparse grouped-GEMM MoE expert FFN with vmap-grad support (DP-SGD). Same
    ``.apply`` signature as the dense ``Opake_MoE`` and Liger's fused op."""
...
def opake_fused_moe(x, gate_up_proj, down_proj, top_k_index, top_k_weights):
    ...
    return Opake_FusedMoE.apply(x, gate_up_proj, down_proj, top_k_index, top_k_weights)
