...
DP-BC machinery from :mod:`opake.optimizers._adam` ports across
...
DP-aware modes mirror :func:`opake.optimizers._adamw`:
...
from opake.api.optimizers._bias_correction import (
    init_per_group_phi,
    is_per_group,
...
if TYPE_CHECKING:
    from opake.pytree import ParamPath
    from opake.types import PerGroup, TensorPytree
...
class RAdamState:
    ...
    Same shape as :class:`opake.optimizers.AdamState` — ``phi`` rides
    ...
    def update_fn(
        ...
        *,
        ...
        if isinstance(new_phi, dict):
            from opake.api.optimizers._bias_correction import map_leaves_with_path
