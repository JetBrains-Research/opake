"""Tests for ``opake.transformers.trl.SFTConfig.from_trl``."""
...
# TRL is the optional ``opake[trl]`` extra. Skip the entire module when
# unavailable so test runs without ``trl`` installed don't error out.
trl = pytest.importorskip("trl")
_SFT_FIELDS = trl.SFTConfig.__dataclass_fields__

from opake.transformers.trl import SFTConfig  # noqa: E402
...
# ---------------------------------------------------------------------------
# REJECT_IF_SET — TRL fields opake doesn't implement
# ---------------------------------------------------------------------------
...
def test_router_aux_loss_is_dropped_with_a_warning(tmp_path):
    ...
    Opake cannot compute the router load-balancing term — it is coupled
...
def test_router_aux_loss_switched_off_is_dropped_silently(tmp_path):
    ...
    requests exactly what opake does anyway.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        cfg = SFTConfig.from_trl(
...
def test_truncation_mode_keep_start_silently_accepted(tmp_path):
    """``keep_start`` is the only opake-supported mode; should be silent."""
    cfg = SFTConfig.from_trl(
        _trl_args(tmp_path, truncation_mode="keep_start"),
        privacy_noise_multiplier=0.8,
        clipping_norm=1.0,
    )
    # keep_start is opake's only supported value — silently accepted.
    assert cfg is not None
...
def test_inherits_max_grad_norm_to_clipping_norm(tmp_path):
    """HF ``max_grad_norm`` flows through the base manifest to opake
...
def test_inherits_perf_kernels_off_by_default(tmp_path):
    ...
    even though opake's own SFT default is True. A name override wins."""
    cfg = SFTConfig.from_trl(
        _trl_args(tmp_path),
        privacy_noise_multiplier=0.8,
        clipping_norm=1.0,
...
def test_inherits_liger_to_perf_kernels(tmp_path):
    """HF ``use_liger_kernel`` flows through to opake
    ``use_performance_kernels=True``."""
    cfg = SFTConfig.from_trl(
