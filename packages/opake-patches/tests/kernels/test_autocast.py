"""``torch.autocast`` over the opake-patches Triton kernels.

Validates that the public ``opake_*`` wrappers honor an active
...
hybrid graph (``nn.Linear`` runs in autocast dtype, ``opake_rms_norm``
...
from opake.patches.kernels import (
...
pytestmark = [
    pytest.mark.cuda,
    pytest.mark.skipif(
        not torch.cuda.is_available(), reason="autocast + opake kernels require CUDA"
    ),
]
...
def test_opake_rms_norm_returns_autocast_dtype(amp_dtype: torch.dtype):
    x = torch.randn(2, 3, 16, device="cuda")
    w = torch.ones(16, device="cuda")
    with torch.autocast(device_type="cuda", dtype=amp_dtype):
        y = opake_rms_norm(x, w)
    assert y.dtype == amp_dtype, f"expected {amp_dtype} under autocast, got {y.dtype}"


@pytest.mark.parametrize("amp_dtype", [torch.float16, torch.bfloat16])
def test_opake_swiglu_returns_autocast_dtype(amp_dtype: torch.dtype):
...
@pytest.mark.parametrize("amp_dtype", [torch.float16, torch.bfloat16])
def test_opake_geglu_returns_autocast_dtype(amp_dtype: torch.dtype):
    g = torch.randn(2, 3, 32, device="cuda")
    u = torch.randn(2, 3, 32, device="cuda")
    with torch.autocast(device_type="cuda", dtype=amp_dtype):
        y_exact = opake_geglu_exact(g, u)
        y_approx = opake_geglu_approx(g, u)
    assert y_exact.dtype == amp_dtype
    assert y_approx.dtype == amp_dtype


@pytest.mark.parametrize("amp_dtype", [torch.float16, torch.bfloat16])
def test_opake_fused_add_rms_norm_returns_autocast_dtype(amp_dtype: torch.dtype):
    x = torch.randn(2, 3, 16, device="cuda")
    res = torch.randn(2, 3, 16, device="cuda")
    w = torch.ones(16, device="cuda")
    with torch.autocast(device_type="cuda", dtype=amp_dtype):
        out, residual = opake_fused_add_rms_norm(x, res, w)
...
class _Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = opake_rms_norm(x, self.norm_w)
        gate = self.gate(x)
        up = self.up(x)
...
def test_block_under_autocast_uniform_intermediate_dtype(amp_dtype: torch.dtype):
    ...
    def tap_forward(inp):
        ...
        x = opake_rms_norm(inp, model.norm_w)
...
# ----------------------------------------------------------------------------
# Backward dtype edge case: autocast Linear → opake kernel → autocast Linear
# ----------------------------------------------------------------------------
...
def test_kernels_autocast_under_compile(backend: str, amp_dtype: torch.dtype):
    ...
    over ``vmap(grad)`` + opake kernels. Compiled grads ≈ eager grads.
    """
    torch.manual_seed(0)
    model = _Block().cuda()
    x = torch.randn(4, 7, 16, device="cuda")
