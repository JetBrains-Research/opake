...
| `router_aux_loss_coef` | `0.0`. TRL's coefficient of the MoE load-balancing term. Non-zero on a mixture-of-experts model trains with the [MoE load-balancing](../../mechanisms/dp-sgd/moe-load-balancing.md) release (`moe_clipped_grad` + `moe_aux`) and this coefficient on its surrogate; zero turns the model's own auxiliary loss off for the run. Ignored by dense models. Requires `privacy_noise_mechanism="gaussian"` and `clipping_mode="fixed"`. |
| `router_aux_kwargs` | `moe_clipped_grad` kwargs: `max_tokens` (the collator's row length; required unless `SFTConfig` derives it from `max_length`, or `DPOConfig` from twice `max_length` for the preference pair), `ratio` (`0.02`, the share of the whitened sensitivity given to the load release), `mean_tokens`, `filter_beta`. |
