...
# then reworked around Opake's functional per-example DP training flow.
...
from opake.dpsgd.clipping import adaptive_clipped_grad, auto_clipped_grad
...
from opake.profiling import PerfTracker, perf_tracker
from opake.random import fold_in, key, split
from opake.random.types import RngKey
from opake.serialization import (
    from_state_dict as opake_from_state_dict,
)
from opake.serialization import (
    state_dict as opake_state_dict,
)
...
class DPTrainer:
    ...
    def __init__(
        ...
        self._apply_opake_model_patches()
        ...
        # Opake step-level performance tracker: per-step wall-clock,
    ...
    def _apply_opake_model_patches(self) -> None:
        ...
        try:
            ...
            log.debug("opake.patches unavailable; skipping model patches.")
            return
    ...
    def _workaround_mps_bf16_sdpa(self) -> None:
        ...
        (:func:`opake.device.sdpa_autocast_under_vmap_broken`) so this auto-drops
    ...
    def _setup_training(
        ...
        *,
        ...
        # shard the dataset (``opake.distributed.local_shard``) and run the
        ...
        if isinstance(mgn, dict):
            from opake.api.engine.clipping import per_group as per_group_clipper
        ...
        # consumers (e.g. opake optimizers' DP bias correction).
        if mechanism_kind == "gaussian":
    ...
    def _inner_training_loop(
        ...
        *,
        ...
        # via the opake.serialization registry; the restored sampler
    ...
    def training_step(
        ...
        with self._perf_tracker.train(batch_size=step_batch_size) as sp:
            ...
            if self._ddp.is_distributed:
                from opake.distributed import sum_gradients_
                from opake.distributed import sync as _opake_sync

                sum_gradients_(grads)
                ctx.clip_state, aux = _opake_sync(ctx.clip_state, aux)
            sp.mark("clip")
    ...
    def compute_per_example_loss(
        ...
        *,
        ...
        Opake CE kernels (which accept ``label_smoothing`` natively —
        ...
        Args:
            fmodel: Functional model from
                :func:`opake.functional.make_functional` (called as
        ...
        # Opake CE kernels (both non-fused and fused-linear) apply it
    ...
    def evaluation_loop(
        ...
        *,
        ...
        :class:`~opake.api.transformers.trainer._eval._PredictionAccumulator`;
        ...
        if self._ddp.is_distributed:
            from opake.api.engine.distributed._state import reduce_scalar
        ...
        # ``{prefix}_<key>``.
        if self._ddp.is_distributed:
            from opake.api.engine.distributed._state import gather_pytree
    ...
    def push_to_hub(
        ...
        **kwargs: Any,
        ...
        model card (with the Opake DP ε/δ section), then uploads
    ...
    def create_model_card(
        ...
        Mirrors ``Trainer.create_model_card`` with Opake-specific additions:
        ``differential-privacy`` + ``opake`` tags, and a ``## Privacy budget``
    ...
    def _resolve_collate_fn(self, base_collator: Callable | None = None) -> Callable:
        ...
        Wraps the HF collator with :func:`opake.functional.empty_collate` so
        ...
        main process.
        """
        from opake.functional import empty_collate
    ...
    def _set_signature_columns_if_needed(self) -> None:
        ...
        # Opake's import-time model patches may wrap ``forward`` into a generic
    ...
    def _build_per_example_loss(
        ...
        *,
        ...
        Args:
            fmodel: Functional model from
                :func:`opake.functional.make_functional`.
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        # (``opake.distributed.local_shard``) and runs the Poisson sampler
    ...
    def get_eval_dataloader(self, eval_dataset: Dataset | None = None) -> DataLoader:
        ...
        per-rank slice via ``opake.distributed.local_shard`` so each rank
    ...
    def create_optimizer(
        ...
        Dispatches via :func:`opake.api.transformers.trainer._optim.build_optimizer`,
        which resolves ``args.optim`` (canonical opake name or HF alias)
        and forwards HF-canonical fields (``learning_rate``,
        ``weight_decay``, ``adam_beta1``/``adam_beta2``, ``adam_epsilon``)
        to the underlying opake factory.  Opake-only knobs (for example
        ...
        string, None} to ``dict[str, Any] | None``.  Opake factories
    ...
    def create_scheduler(self, num_training_steps: int) -> Callable[[int], float]:
        ...
        Dispatches via :func:`opake.api.transformers.trainer._scheduler.build_lr_schedule`,
    ...
    def _maybe_log_save_evaluate(
        ...
        *,
        ...
        if ctrl.should_log:
            ...
            # Opake per-step performance metrics (step_time_sec,
    ...
    def _save_checkpoint(self, model: Any = None, trial: Any = None) -> str:
        ...
        an adapter. ``model`` and ``trial`` are accepted and ignored: opake
        ...
        """
        del model, trial  # HF parity; opake uses ``self._ctx`` / ``self._model``.
        ctx = self._ctx
        if ctx is None:
            raise OperationError(
                *(
        ...
        if _distributed.should_save(a, self._ddp):
            try:
                if Path(ckpt_dir).is_dir():
                    # Defensive: the only callers target a fresh step, but
                    # never let a stale dir block the atomic rename.
    ...
    def _save_dp_runtime(self, ckpt_dir: str, ctx: _TrainingContext) -> None:
        ...
        sampler_state = (
            opake_state_dict(ctx.current_sampler)
            if ctx.current_sampler is not None
            else None
        )
        ...
        ckpt.save_dp_runtime_state(
            ...
            horizon_process_state=(
                opake_state_dict(ctx.horizon_process)
    ...
    def _save_accountant(self, ckpt_dir: str, accountant: Accountant) -> None:
        ...
        with path.open("w") as f, _deep_json_recursion():
            json.dump(opake_state_dict(accountant), f)
    ...
    def _read_runtime_for_resume(
        ...
        if self.args.privacy_accounting:
            with (
                ...
                accountant = opake_from_state_dict(Accountant(), json.load(f))
    ...
    def _apply_runtime_state(
        ...
        ctx.clip_state = opake_from_state_dict(ctx.clip_state, runtime.clip_state)
        ctx.noise_state = opake_from_state_dict(ctx.noise_state, runtime.noise_state)
        ...
        if opt_path.exists():
            ...
            ctx.opt_state = opake_from_state_dict(ctx.opt_state, opt_sd)
            if isinstance(ctx.opt_state, ScheduleFreeState):
                ctx.trainable_params = {
    ...
    def _warn_on_arg_drift(self, runtime: ckpt.RuntimeCheckpoint) -> None:
        ...
        :class:`~opake.api.transformers.trainer._checkpoint.RuntimeCheckpoint`
        for the vocabulary):
