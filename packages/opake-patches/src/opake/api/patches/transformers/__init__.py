...
families::
    ...
    from opake.api.patches.transformers import (make_apply_family_patches, make_apply_model_patches, register_activation_kind)
...
``opake.patches.apply_model_patches(model)`` is enough — it discovers
the family from the model and dispatches to the right
``apply_X_patches``.  This module is for users who:

- ship their own architecture and want it to participate in opake's
...
from opake.api.patches.transformers import models  # noqa: F401
from opake.api.patches.transformers._factory import (
...
from opake.api.patches.transformers._registry import (
    register_family,
    supported_families,
)
