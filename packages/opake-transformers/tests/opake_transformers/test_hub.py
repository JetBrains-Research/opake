...
- ``create_model_card`` writes ``README.md`` containing ``differential-privacy``
  and ``opake`` tags and the ``## Privacy budget`` section.
...
import opake.api.transformers.trainer._hub as _hub
...
class TestInitHfRepo:
    def test_called_at_construction(self, tmp_path):
        ...
        with patch(
            "opake.api.transformers.trainer._hub._create_repo", return_value=mock_url
        ) as mock_create:
...
class TestPushToHub:
    def _run_push(self, tmp_path, **overrides):
        ...
        with (
            ...
            patch("opake.api.transformers.trainer._hub.create_model_card") as mock_card,
            patch("opake.api.transformers.trainer._hub._upload_folder") as mock_upload,
        ):
    ...
    def test_non_blocking_sets_run_as_future(self, tmp_path):
        ...
        with (
            ...
            patch("opake.api.transformers.trainer._hub.create_model_card"),
            patch("opake.api.transformers.trainer._hub._upload_folder") as mock_upload,
        ):
...
class TestCreateModelCard:
    def _build_card(self, tmp_path, log_history=None):
        ...
        with patch(
            "opake.api.transformers.trainer._hub._create_repo", return_value=model_url
        ):
            trainer = _tiny_trainer(
                tmp_path,
                push_to_hub=True,
    ...
    def test_dp_section_idempotent(self, tmp_path):
        ...
        assert content2.count("<!-- opake-dp:begin -->") == 1
        assert content2.count("<!-- opake-dp:end -->") == 1
...
class TestSpliceDpSection:
    def test_inserts_after_framework_versions(self):
        ...
        section = "<!-- opake-dp:begin -->\n### Privacy budget\nDP stuff\n<!-- opake-dp:end -->"
        result = _hub._splice_dp_section(card, section)
        fw_pos = result.index("### Framework versions")
        opake_pos = result.index("- Opake ")
        dp_pos = result.index("<!-- opake-dp:begin -->")
        assert dp_pos > fw_pos, "DP section should appear after Framework versions"
        assert opake_pos > fw_pos, (
            "Opake version should be listed under Framework versions"
        )
        assert opake_pos < dp_pos, (
            "Opake framework version should appear before DP section"
        )
        assert result.count("<!-- opake-dp:begin -->") == 1

    def test_framework_versions_idempotent_when_opake_present(self):
        card = (
            "## Training procedure\n\n"
            "### Framework versions\n\n"
            "- Transformers 4.x\n"
    ...
    def test_replaces_existing_section(self):
        old_section = "<!-- opake-dp:begin -->\nOLD\n<!-- opake-dp:end -->"
        card = f"# Model\n\n{old_section}\n"
        new_section = "<!-- opake-dp:begin -->\nNEW\n<!-- opake-dp:end -->"
...
class TestSaveModelPush:
    def test_save_model_triggers_push_when_push_to_hub(self, tmp_path):
        ...
        with (
            ...
            patch("opake.api.transformers.trainer._hub.push_to_hub") as mock_push,
        ):
