...
:class:`opake.transformers.trainer.DPTrainer` for model weights). Native dtype
...
# the OS reclaim it.
_DEFAULT_CACHE_SUBDIR = "opake_ref_cache"
...
def _cache_path(
    ...
    """Resolve the ``.safetensors`` path, defaulting to ``<tmp>/opake_ref_cache``."""
    ...
    **Outside vmap only** — see the module docstring.
    ...
    Args:
        ...
        cache_dir: Directory for the ``.safetensors`` cache. Defaults to
            ``<tempdir>/opake_ref_cache`` (created on first miss).
