"""Shared TRL → opake conversion plumbing.
...
:mod:`opake.api.transformers.trainer._hf_convert` and only classifies
...
gate (``trl`` is the ``pip install opake[trl]`` extra), the rejectors common
...
def _import_trl() -> Any:
    ...
    try:
        ...
        raise ImportError(  # noqa: TRY003 - preserve standard Python error contract
            ...
            "dependency. Install with ``pip install opake[trl]`` (or "
...
def _reject_truncation_mode(value: Any) -> str | None:
    if value == "keep_end":
        return (
            "TRL deprecated ``truncation_mode='keep_end'``; opake only "
...
def _drop_router_aux_loss(value: Any) -> str | None:
    ...
    return (
        ...
        "per-example gradient to clip, so it cannot enter opake's DP "
...
def _convert_trl_config(
    ...
    *,
    ...
    """Shared TRL → opake conversion dispatcher.
    ...
    # Performance kernels default ON in opake but OFF in HF/TRL; default OFF on
