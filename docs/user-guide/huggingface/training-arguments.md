...
| `router_aux_kwargs` | `moe_clipped_grad` kwargs: `max_tokens` (the collator's row length; required unless `SFTConfig` derives it from `max_length`), `ratio` (`0.02`, the share of the whitened sensitivity given to the load release), `mean_tokens`, `filter_beta`. |
