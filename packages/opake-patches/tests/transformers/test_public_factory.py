...
from opake.api.patches.transformers._registry import _FAMILY_REGISTRY
from opake.patches.transformers import (
    family_name,
...
def test_built_in_kinds_compose_into_a_callable():
    ...
    assert apply._opake_family == "public_api_test_a"
...
def test_activation_kwarg_gates_family_selected_activation_patch(monkeypatch):
    ...
    assert calls == []
    assert not hasattr(FakeMLP.forward, "__opake_patched__")

    apply(performance=True, compat=True, activation=True)
    assert len(calls) == 1
    assert getattr(FakeMLP.forward, "__opake_patched__", False)
...
def test_cross_entropy_sets_loss_function_on_model_instance(monkeypatch):
    ...
    assert patched.loss_function is _opake_causal_lm_loss
    assert untouched.loss_function is original_loss


def test_fused_linear_cross_entropy_follows_performance(monkeypatch):
    """The conditional forward follows ``performance`` and supports overrides."""
    import torch
    ...
    monkeypatch.setattr("opake.api.engine.device.fused_kernels_available", lambda: True)
    ...
    # The wrapper follows the normal performance bucket.
    _, FakeA, _original_loss, apply_a = _fresh_module("a")
    instance_a = FakeA()
    apply_a(instance_a, performance=True, compat=False)
    assert instance_a.loss_function is _opake_causal_lm_loss
    assert getattr(FakeA.forward, "__opake_patched__", False)

    # Turning off performance keeps both CE patches off by default.
    _, FakeB, original_loss_b, apply_b = _fresh_module("b")
    instance_b = FakeB()
    apply_b(instance_b, performance=False, compat=False)
    assert instance_b.loss_function is original_loss_b
    assert not hasattr(FakeB.forward, "__opake_patched__")
    ...
    assert not hasattr(FakeC.forward, "__opake_patched__")

    _, FakeOverride, _, apply_override = _fresh_module("override")
    instance_override = FakeOverride()
    apply_override(
        instance_override,
        performance=False,
        compat=False,
    ...
    assert getattr(FakeOverride.forward, "__opake_patched__", False)
    ...
    assert not hasattr(FakeD.forward, "__opake_patched__")
    ...
    monkeypatch.setattr(
        "opake.api.patches.transformers._factory._make_fused_ce_causal_lm_forward",
        record_chunk_width,
    )
    patched = []
    monkeypatch.setattr(
        "opake.api.patches.transformers._factory._patch_forward",
...
def test_family_factory_default_replacements_are_opake_vmap_safe():
    """Default closures bind opake's vmap-safe implementations."""
    from opake.api.patches.transformers._family import (
        _opake_apply_rotary_pos_emb,
    ...
    assert _opake_apply_rotary_pos_emb in closure
...
def test_family_factory_tracks_idempotency_per_enabled_concern(monkeypatch):
    from opake.api.patches.transformers._family import _reset_patched_families
...
def test_register_family_routes_via_apply_transformers_model_patches():
    ...
    :func:`opake.patches.apply_model_patches`) consults the registry,
    so user-registered families are reached without any source-code
    changes inside opake-patches."""
    from opake.api.patches.transformers._router import apply_transformers_model_patches
