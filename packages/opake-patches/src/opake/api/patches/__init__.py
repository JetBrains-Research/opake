...
def apply_model_patches(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.debug("opake: Hugging Face kernel patches not available.")
...
def apply_runtime_patches(
    *, performance: bool = True, compat: bool = True, **kwargs
    ...
    if vmap_grouped_mm:
        try:
            from opake.api.patches.transformers.runtime.moe import (
    ...
    if empty_batches:
        try:
            from opake.api.patches.transformers.runtime.collator import (
    ...
    if vmap_checkpointing:
        try:
            from opake.api.patches.torch import apply_checkpoint_patch

            apply_checkpoint_patch(vmap_checkpointing=vmap_checkpointing)
        except ImportError:
            logger.warning(
                "opake: checkpoint+functorch patches unavailable; "
