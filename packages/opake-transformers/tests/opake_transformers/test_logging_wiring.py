...
from opake.api.transformers.trainer._distributed import DDPState, apply_logging
...
def restore_logging():
    ...
    opake_logger = logging.getLogger("opake")
    saved_level = opake_logger.level
    try:
        yield
    finally:
        hf_logging.set_verbosity(saved_verbosity)
        opake_logger.setLevel(saved_level)
