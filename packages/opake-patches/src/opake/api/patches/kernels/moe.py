...
Opake pattern for ``vmap(grad)`` (DP-SGD per-sample gradients). The vmap rules
...
class Opake_MoE(torch.autograd.Function):
...
def opake_moe(x, gate_up_proj, down_proj, top_k_index, top_k_weights, *, grouped=True):
    ...
    suited to the host; ``grouped=False`` forces the dense ``Opake_MoE``
    **compat** path on every host. The patch layer wires ``grouped`` from the
    ...
    Both grouped-GEMM paths are performance variations — only ``Opake_MoE`` is
    ...
    from ._grouped_moe import Opake_GroupedMoE, grouped_mm_available
    ...
    if decision.backend == "triton":
        from .fused_moe import Opake_FusedMoE

        return Opake_FusedMoE.apply(
            x, gate_up_proj, down_proj, top_k_index, top_k_weights
        )
    if decision.backend == "grouped":
        return Opake_GroupedMoE.apply(
            x, gate_up_proj, down_proj, top_k_index, top_k_weights
        )
    return Opake_MoE.apply(x, gate_up_proj, down_proj, top_k_index, top_k_weights)
