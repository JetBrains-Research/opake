...
def test_chosen_nll_is_a_reference_free_head():
    # ``chosen_nll`` (opake's name for TRL's ``sft`` head) scores the policy's
...
def test_dpo_tr_dpo_syncs_reference(tmp_path, monkeypatch):
    ...
    from opake.api.alignment.dpo.reference import _precompute
    ...
    assert not (tmp_path / "opake_ref_cache").exists()
...
def test_dpo_simpo_loss_matches_formula(tmp_path):
    ...
    from opake.alignment.dpo.loss import sequence_logp, simpo_loss
...
def test_dpo_orpo_loss_matches_formula(tmp_path):
    ...
    # length-normalized, reference-free policy logps.
    from opake.alignment.dpo.loss import odds_ratio_loss, sequence_logp
...
def test_dpo_model_cache_identity_tracks_reference_state():
    from opake.api.transformers.trl._dpo_trainer import (
...
def test_dpo_disabled_adapter_identity_ignores_adapter_weights():
    from opake.api.transformers.trl._dpo_trainer import _model_cache_identity
...
def test_sft_resolve_fused_handles_unwraps_peft():
    from opake.api.transformers.trl._sft_trainer import _resolve_fused_handles
...
def test_dpo_resolve_fused_handles_unwraps_peft():
    from opake.api.transformers.trl._dpo_trainer import _resolve_fused_handles
