"""DDP runtime support for :class:`opake.transformers.trainer.DPTrainer`.
...
``__init__`` device pick, and use :mod:`opake.distributed` once the
process group is up.
"""
...
from opake.distributed import get_rank, get_world_size, is_distributed
from opake.distributed.collectives import all_reduce_ as _opake_all_reduce_
from opake.distributed.collectives import barrier as _opake_barrier
...
def apply_logging(args: Any, ddp: DDPState) -> int:
    ...
    transformers-namespace logger and opake's root logger, so
    ...
    if raw != -1:
        ...
        logging.getLogger("opake").setLevel(raw)
    return raw
...
def checkpoint_barrier(ddp: DDPState, local_error: Exception | None) -> None:
    ...
    _opake_all_reduce_(failed, op="max")
    if local_error is not None:
        raise local_error
