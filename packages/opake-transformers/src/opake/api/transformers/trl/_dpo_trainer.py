...
Opake's per-example :meth:`Trainer.compute_per_example_loss_and_metrics` seam
...
from opake.api.transformers.trainer import Trainer
from opake.api.transformers.trainer._distributed import resolve_ddp_state

# Single source of truth for PEFT detection (handles PeftModel + PeftMixedModel).
from opake.api.transformers.trainer._trainer import _is_peft_model
...
# TRL ``loss_type`` name → ``opake.alignment.dpo.loss`` head. An unknown value
...
class DPOTrainer(Trainer):
    ...
    def _inject_tr_ref_logps(
        ...
        batch path in :class:`Trainer`); the reference forward is skipped
