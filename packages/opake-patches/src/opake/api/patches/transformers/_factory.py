...
1. An ``apply_X_family_patches`` function (built by
   :func:`opake.patches.transformers._family.make_apply_family_patches`)
...
from opake.api.patches.transformers._router import (
    _has_kernel_runtime,
    _patch_forward,
)
from opake.api.patches.transformers.components.batchify import apply_batchify_patch
...
from opake.api.patches.transformers.components.fused_add_rms_norm import (
    _fused_add_rms_fac_gemma,
    _fused_add_rms_fac_granite,
    _fused_add_rms_fac_llama,
    _fused_add_rms_fac_phi3,
)
from opake.api.patches.transformers.components.geglu import (
    _make_geglu_approx_mlp_forward,
    _make_geglu_exact_mlp_forward,
)
from opake.api.patches.transformers.components.kv_cache import apply_kv_cache_patch
from opake.api.patches.transformers.components.moe import (
    _make_moe_experts_forward,
)
from opake.api.patches.transformers.components.rms_norm import (
    _rmsnorm_fac_gemma,
    _rmsnorm_fac_gemma2,
    _rmsnorm_fac_glm4,
    _rmsnorm_fac_llama,
...
from opake.api.patches.transformers.components.swiglu import (
...
if TYPE_CHECKING:
    from opake.api.patches.transformers.types import (
        FamilyPatchFn,
        ForwardFactory,
        ModelPatchFn,
    )
    ...
    *,
    ...
    Args:
        ...
        moe_kind: Same shape — for the stacked-weight MoE experts module
            ...
            DP-SGD, not a CUDA kernel). ``opake_moe`` transparently uses the
    ...
    def apply(
        ...
        **kwargs: object,
        ...
        #      ``grouped_moe=False`` forces the dense, always-correct ``Opake_MoE``.
        ...
        #     ``Opake_CrossEntropyLoss`` via ``loss_function``. Operates
    ...
    apply._opake_family = family  # type: ignore[attr-defined]
    return apply
