...
The sparse grouped path must match the dense ``Opake_MoE`` oracle — forward and,
...
from opake.api.patches.kernels import _moe_memory
from opake.api.patches.kernels._grouped_moe import (
    Opake_GroupedMoE,
    _grouped_AtB,
    grouped_mm_available,
)
from opake.api.patches.kernels.moe import Opake_MoE, opake_moe
...
def _check_parity(device: str) -> None:
    ...
    o_s = Opake_GroupedMoE.apply(x2, gu, dn, idx2, w2)
    o_d = Opake_MoE.apply(x2, gu, dn, idx2, w2)
    ...
    def f_s(xx, gg, dd, ii, ww):
        return Opake_GroupedMoE.apply(xx, gg, dd, ii, ww).sum()

    def f_d(xx, gg, dd, ii, ww):
        return Opake_MoE.apply(xx, gg, dd, ii, ww).sum()
...
def _check_bf16_within_floor(device: str) -> None:
    ...
    assert (
        _frob(
            Opake_GroupedMoE.apply(x2, gu, dn, idx2, w2),
            Opake_MoE.apply(x2, gu, dn, idx2, w2),
        )
        < 1e-2
    ), "forward"

    def f_s(xx, gg, dd, ii, ww):
        return Opake_GroupedMoE.apply(xx, gg, dd, ii, ww).float().sum()

    def f_d(xx, gg, dd, ii, ww):
        return Opake_MoE.apply(xx, gg, dd, ii, ww).float().sum()
...
def _check_dispatch(device: str) -> None:
    ...
    for E in (4, 16):
        ...
        out = opake_moe(x2, gu, dn, idx2, w2)
        ref = Opake_MoE.apply(x2, gu, dn, idx2, w2)
...
def _check_grouped_gate(device: str) -> None:
    # ``grouped=False`` forces the dense ``Opake_MoE`` compat path even for large
    ...
    ref = Opake_MoE.apply(x2, gu, dn, idx2, w2)
    dense = opake_moe(x2, gu, dn, idx2, w2, grouped=False)
    grouped = opake_moe(x2, gu, dn, idx2, w2, grouped=True)
...
def test_grouped_moe_trainable_backward_stays_near_budget(monkeypatch):
    ...
    from opake.profiling import get_memory_stats, reset_peak_memory
...
def test_grouped_moe_chunking_reduces_mps_forward_peak(monkeypatch):
    ...
    def run():
        return Opake_GroupedMoE.apply(x, gate_up, down, index, weights)
