"""Optimizer name validation, HF-alias remapping, and the opake set.

Trainer's optimizer surface has two layers:

1. Canonical opake names (``adam``, ``adamw``, ``sgd``, ``rmsprop``,
   ``adagrad``, ``adafactor``, ``ademamix``, ``lion``, ``radam``,
   ``adadelta``, ``schedule_free``) route directly to ``opake.optimizers.*``
   factories
...
2. HF compat aliases (``adamw_torch``, ``adamw_torch_fused``,
   ...
   ``schedule_free_radam``) route to the same opake factories — Trainer
...
from opake.api.transformers.trainer._optim import (
    build_optimizer,
...
# opake factory.  Construction must succeed; the underlying update
# math is the matching opake factory, *not* the HF-named impl.
ACCEPTED_HF_ALIASES = (
    "adamw_torch",  # ↦ opake.optimizers.adamw
    "adamw_torch_fused",  # ↦ opake.optimizers.adamw
    "adamw_hf",  # ↦ opake.optimizers.adamw
    "adafactor",  # ↦ opake.optimizers.adafactor
    "ademamix",  # ↦ opake.optimizers.ademamix
    "lion_32bit",  # ↦ opake.optimizers.lion
    "schedule_free_radam",  # ↦ schedule_free(radam(...))
)


# Names that Trainer cannot honour even with a remap: 8-bit / paged /
# GaLore / Apex-fused / XLA / NPU / opake primitives without DP-aware
...
class TestOptimAcceptsHFAliases:
    """HF compat aliases route onto the matching opake factory."""
...
class TestOptimRejectsUnsupportedNames:
    ...
    def test_unsupported_optim_raises(self, tmp_path, name):
        with pytest.raises(ValueError, match="is not supported by Trainer") as exc_info:
...
# Canonical opake names Trainer ships.  ``schedule_free`` is not in
...
class TestOptimizerClsAndKwargsFunctional:
    """``optimizer_cls_and_kwargs`` accepts opake/torchopt-style factories only."""
    ...
    def test_trainer_constructor_stores_functional_factory(self, tmp_path):
        ...
        trainer = Trainer(
            ...
            args=args,
            optimizer_cls_and_kwargs=(opake_opt.adamw, {"weight_decay": 0.01}),
        )
