...
# DP-meaningful subset used by Opake's trainer stack.
...
- ``clipping_norm`` is the **per-example DP clipping bound**. Pass a
  ...
  pattern keys for :func:`opake.api.engine.clipping.per_group` semantics. Adaptive /
...
from opake.api.engine.device import device_capabilities
...
# :mod:`opake.dpftrl.noise`, dispatched through ``_dpftrl.build_strategy``
...
# ``CyclicPoissonSampler`` realises and ``opake.dpftrl.accounting.poisson``
# accounts for.
...
# :mod:`opake.dpftrl.noise` exactly so the trainer can spread the dict
...
class TrainingArguments:
    ...
    # *or* an :data:`~opake.scheduling.types.Schedule` recipe directly.
    ...
    # =================================================================
    local_rank: int = -1
    # Distributed backend policy for current Opake support surface.
    ...
    # compatible with Opake's sequence-sensitive sampling modes.
    ...
    # ``report_to=['trackio']`` callback path that opake inherits.  Mirrors
    ...
    # ``opake.patches.apply_model_patches`` kwargs (no key translation).
    ...
    # Whether ``opake.patches.apply_model_patches`` should apply compat
    ...
    # opake's patches, or for non-HF ``nn.Module`` test fixtures —
    # silences the "no detectable family" info log opake emits when it
    ...
    # calls ``trainer._save_checkpoint(model, trial)``; opake's override
    ...
    def __post_init__(self) -> None:
        ...
        if (self.bf16 or self.bf16_full_eval) and not self.use_cpu:
            ...
            # ``transformers``, so consult opake's capability probe too — this
        ...
        if (
            ...
            log.warning(
                ...
                "runtime stack that is not covered by Opake first-class CI. "
    ...
    def warmup_ratio(self) -> float:
        ...
        still reads it; opake follows 5.x, where a ``warmup_steps`` below
        1 *is* the fraction.
        """
    ...
    def from_hf(
        ...
        **opake_overrides: Any,
        ...
        """Convert an HF ``TrainingArguments`` to opake ``TrainingArguments``.

        Required: exactly one of ``privacy_noise_multiplier=<float>``
        (fixed-noise mode) or ``privacy_target_epsilon=<float>`` (calibrated
        noise) must be set — opake's runtime requires one to instantiate.

        Translates HF fields by bucketed manifest (see
        ``opake.api.transformers.trainer._hf_convert``):
        ...
        - **TRANSFORM**: HF's ``(per_device_train_batch_size, gradient_accumulation_steps)``
          collapses into opake's ``per_device_train_batch_size = product``
        ...
        - **DROP_WITH_WARN**: silently drops HF fields that have no opake
        ...
        in opake, that's ``per_device_train_batch_size`` alone. The
        converter collapses HF's two-field expression into opake's
        ...
        ``opake_overrides`` allow setting any opake-only field
        ...
        # value derived from HF ``max_grad_norm`` (or opake's own default)
        # stands.
        ...
        # ``opake_overrides`` win over both HF-derived values and DP defaults.
        converted.update(opake_overrides)
