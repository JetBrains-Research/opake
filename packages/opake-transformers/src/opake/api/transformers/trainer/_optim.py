...
1. **Canonical opake names** — every factory in
   :mod:`opake.optimizers` (``adam``, ``adamw``, ``sgd``, ``rmsprop``,
   ``adagrad``, ``adafactor``, ``ademamix``, ``lion``, ``radam``,
   ``adadelta``, ``schedule_free``).
   The full opake parameter surface is reachable through HF-canonical
   ...
   ``update_rms_clip``, and other opake optimizer knobs.  Opake factories
   raise ``TypeError`` on unknown keys.

2. **HF compat aliases** — HF's ``OptimizerNames`` values that map
   cleanly onto an opake factory (``adamw_torch`` → ``adamw``,
   ...
   honoured by selecting the opake factory whose math matches.
...
import opake.optimizers as opake_opt
from opake.exceptions import ConfigurationError, InputTypeError, OperationError
...
# ---------------------------------------------------------------------------
# Layer 1 — canonical opake names
# ---------------------------------------------------------------------------

_OPAKE_FACTORIES: dict[str, Callable[..., GradientTransformation]] = {
    "adam": opake_opt.adam,
    "adamw": opake_opt.adamw,
    "sgd": opake_opt.sgd,
    "lion": opake_opt.lion,
    "ademamix": opake_opt.ademamix,
    "adafactor": opake_opt.adafactor,
    "rmsprop": opake_opt.rmsprop,
    "adagrad": opake_opt.adagrad,
    "radam": opake_opt.radam,
    "adadelta": opake_opt.adadelta,
    "schedule_free": opake_opt.schedule_free,
}
...
# HF name → (opake factory key, base kwargs).  ``base kwargs`` apply
...
_HF_ALIASES: dict[str, tuple[str, dict[str, Any]]] = {
    ...
    "ademamix": ("ademamix", {}),
    "lion_32bit": ("lion", {}),
    # ``lion`` itself is a canonical opake name; HF's enum still
...
def supported_names() -> tuple[str, ...]:
    """Sorted union of canonical opake names + HF aliases."""
    return tuple(sorted(set(_OPAKE_FACTORIES) | set(_HF_ALIASES)))


def canonical_optimizer_names() -> tuple[str, ...]:
    """Sorted canonical opake optimizer names (no HF aliases)."""
    return tuple(sorted(_OPAKE_FACTORIES))
...
def resolve_optimizer_name(optim: Any) -> tuple[str, dict[str, Any]]:
    ...
    Whitelist-based: only canonical opake names and the HF aliases we
...
def _apply_top_level_fields(
    ...
    if canonical in _APPLIES_BETAS:
        ...
        # β₃ to the opake factory's default; users can override the
...
def _build_schedule_free(
    ...
    # introspection; opake.optimizers.schedule_free raises on unknown
    # keys, so we hand it everything left after building the base.
    base_factory = _OPAKE_FACTORIES[base_name]
    base_only = {}
    ...
    return opake_opt.schedule_free(base, **pooled)
...
def build_optimizer(
    ...
    """Construct the DP-aware opake optimizer for ``args.optim``.
    ...
    factory = _OPAKE_FACTORIES[canonical]
    kwargs = {**base_kwargs}
...
def validate_functional_optimizer_cls_and_kwargs(
    ...
    convention as :mod:`opake.optimizers` and torchopt).  The returned
