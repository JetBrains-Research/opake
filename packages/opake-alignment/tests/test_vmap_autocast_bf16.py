...
The opake-alignment training path runs each per-example loss closure inside
...
1. ``vmap(grad(fused_*))`` under ``autocast(bf16)`` produces finite gradients
   ...
   opake-patches Triton path and must not regress to the broken fused-aten
...
from opake.api.alignment import _fused_lce
from opake.api.alignment.logprob._sequence import fused_sequence_logp
from opake.api.alignment.sft.loss._dft import fused_dft_loss
from opake.api.alignment.sft.loss._nll import fused_nll_loss
