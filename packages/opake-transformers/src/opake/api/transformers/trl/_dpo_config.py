...
# Opake's per-example DP training flow.
...
extending Opake's standalone
:class:`~opake.api.transformers.trainer._training_arguments.TrainingArguments`. Batch-coupled
...
from opake.api.transformers.trainer._training_arguments import TrainingArguments
...
class DPOConfig(TrainingArguments):
    ...
    #: Opake-specific (no TRL analogue): enable model-level Triton kernels
    ...
    def from_trl(
        ...
        **opake_overrides: Any,
        ...
        """Convert a ``trl.DPOConfig`` to an opake ``DPOConfig``.

        Requires the optional ``trl`` extra:
        ``pip install opake[trl]``.
        ...
        ``loss_type`` is validated per-element against opake's implemented
        heads; the TRL 1.x ``aot`` / ``aot_unpaired`` Adversarial Optimal
        Transport family raises ``ValueError`` because opake does not
        implement them.

        Fields TRL has that opake does not implement raise
