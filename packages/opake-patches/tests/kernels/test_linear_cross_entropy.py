...
import opake.api.patches.kernels.linear_cross_entropy as linear_ce_mod
from opake.api.patches.kernels.linear_cross_entropy import (
    Opake_LinearCrossEntropyLoss,
...
def opake_linear_ce(
    ...
    """Opake kernel wrapper for functional use in vmap/grad.
    ...
    nll_sum, _lse, _valids, _token_weight = Opake_LinearCrossEntropyLoss.apply(
        hidden_states,
        weight,
        labels,
        ignore_index,
...
class TestLinearCEForward:
    ...
    def test_forward_matches_pytorch(self, assert_precision, mellum_config, vocab_size):
        ...
        out_op = opake_linear_ce(hidden, weight, labels)
    ...
    def test_forward_label_smoothing(self, assert_precision, mellum_config, vocab_size):
        ...
        for ls in (0.05, 0.1):
            ...
            out_op = opake_linear_ce(hidden, weight, labels, label_smoothing=ls)
    ...
    def test_backward_label_smoothing_hidden_grad(
        ...
        loss_op = opake_linear_ce(hidden_op, weight, labels, label_smoothing=ls)
        loss_op.backward()
    ...
    def test_backward_label_smoothing_weight_grad(
        ...
        loss_op = opake_linear_ce(hidden, weight_op, labels, label_smoothing=ls)
        loss_op.backward()
...
class TestLinearCEBackward:
    ...
    def test_backward_hidden_states_grad(
        ...
        hidden_op = hidden_pt.detach().clone().requires_grad_(True)
        loss_op = opake_linear_ce(hidden_op, weight, labels)
        loss_op.backward()
    ...
    def test_backward_weight_grad(self, assert_precision, mellum_config, vocab_size):
        ...
        weight_op = weight_pt.detach().clone().requires_grad_(True)
        loss_op = opake_linear_ce(hidden, weight_op, labels)
        loss_op.backward()
...
def test_bare_grad_matches_reference(assert_precision):
    ...
    got = grad(lambda w: opake_linear_ce(hidden, w, labels))(weight)
...
def test_backward_reuses_forward_statistics(
    ...
    def loss(h, w, lab):
        return Opake_LinearCrossEntropyLoss.apply(
...
class TestLinearCEVmapForward:
    ...
    def test_vmap_forward_precision(self, assert_precision, mellum_config, vocab_size):
        ...
        def f_op(h, t):
            return opake_linear_ce(h, weight, t)
...
class TestLinearCEMemory:
    ...
    def test_hidden_backward_stays_logits_free(self, measure_incremental_cuda_memory):
        ...
        def run_backward():
            loss = opake_linear_cross_entropy_loss(hidden, weight, labels)
...
class TestLinearCEPerformance:
    ...
    def test_forward_performance(
        ...
        def opake_fn(h, w):
            return opake_linear_ce(h, w, labels)

        pt_stats = measure_time_and_memory(pytorch_fn, hidden, weight)
        op_stats = measure_time_and_memory(opake_fn, hidden, weight)
...
class TestLinearCESoftcapping:
    ...
    def test_softcapping_forward(self, assert_precision, mellum_config):
        ...
        out_op = opake_linear_ce(hidden, weight, labels, softcap=softcap)
    ...
    def test_softcapping_backward(self, assert_precision, mellum_config):
        ...
        hidden_op = hidden_pt.detach().clone().requires_grad_(True)
        loss_op = opake_linear_ce(hidden_op, weight, labels, softcap=softcap)
        loss_op.backward()
    ...
    def test_logit_scaling_forward(self, assert_precision, mellum_config):
        ...
        out_op = opake_linear_ce(hidden, weight, labels, scaling=scaling)
    ...
    def test_logit_scaling_backward(self, assert_precision, mellum_config):
        ...
        hidden_op = hidden_pt.detach().clone().requires_grad_(True)
        weight_op = weight_pt.detach().clone().requires_grad_(True)
        loss_op = opake_linear_ce(hidden_op, weight_op, labels, scaling=scaling)
        loss_op.backward()
    ...
    def test_logit_transform_vmap_grad(
        ...
        def f_op(h, t):
            return opake_linear_ce(h, weight, t, softcap=softcap, scaling=scaling)
...
class TestLinearCEWrapper:
    ...
    def test_wrapper_matches_apply(self, assert_precision, mellum_config):
        ...
        out_wrapper = opake_linear_cross_entropy_loss(
            hidden,
            weight,
            labels,
        )
