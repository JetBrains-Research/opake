...
import opake.api.transformers.trainer._checkpoint as ckpt
...
from opake.serialization import from_state_dict as opake_from_state_dict
...
class TestDpRuntimeBundle:
    def test_roundtrip(self, tmp_path):
        ...
        assert opake_from_state_dict(clip, loaded.clip_state) == clip
        assert opake_from_state_dict(noise, loaded.noise_state) == noise
    ...
    def test_rejects_pre_removal_bundle_version(self, tmp_path):
        path = str(tmp_path / "dp.pt")
...
class TestNoiseStreamContinuity:
    ...
    def test_continues_after_resume(self, tmp_path, case):
        ...
        for restore_mode in ("fresh", "poisoned"):
            ...
            resumed_state = opake_from_state_dict(
                state_template,
