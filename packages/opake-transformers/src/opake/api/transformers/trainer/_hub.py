...
- ``create_model_card`` — write ``README.md`` (HF card + Opake DP ε/δ section)
...
_DP_SECTION_MARKER_BEGIN = "<!-- opake-dp:begin -->"
_DP_SECTION_MARKER_END = "<!-- opake-dp:end -->"


def _opake_version() -> str:
    try:
        from importlib.metadata import version

        return version("opake-transformers")
    except Exception:
        return "unknown"
...
class DPTrainingSummary(TrainingSummary):
    """Opake-owned TrainingSummary variant for model-card customization."""
    ...
    def to_model_card(self) -> str:
        ...
        if self.privacy is None:
            return _splice_opake_framework_version(card)
...
def create_model_card(
    ...
    license inference) plus Opake card customization.

    Opake-specific additions:
    - Tags ``"differential-privacy"`` and ``"opake"`` are merged into
    ...
    # Inject Opake tags before delegating to DPTrainingSummary.
    opake_tags = ["differential-privacy", "opake"]
    if tags is None:
        tags = opake_tags
    elif isinstance(tags, str):
        tags = [tags, *opake_tags]
    else:
        for t in opake_tags:
            if t not in tags:
                tags.append(t)
...
def _splice_dp_section(card: str, dp_section: str) -> str:
    """Replace or append the ``<!-- opake-dp:begin/end -->`` block.
    ...
    """
    card = _splice_opake_framework_version(card)
...
def _splice_opake_framework_version(card: str) -> str:
    """Insert Opake version into the Framework versions subsection."""
    m = _FRAMEWORK_VERSIONS_RE.search(card)
    if m is None:
        return card

    framework_block = m.group(1)
    if re.search(r"^- Opake\b", framework_block, re.MULTILINE):
        return card

    opake_line = f"- Opake {_opake_version()}\n"
    insert_pos = m.end()
    return card[:insert_pos] + opake_line + card[insert_pos:]
