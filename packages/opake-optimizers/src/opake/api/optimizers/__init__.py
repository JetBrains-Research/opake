"""Functional optimizers for Opake.

Single import path for every functional optimizer used in the Opake
DP training pipeline — Opake-built factories with a common wrapper-aware
...
- :func:`schedule_free` — generic wrapper around any base
  ``GradientTransformation`` (Opake-built or torchopt-imported)
  ...
  ``state.x`` field (see :class:`opake.optimizers.types.ScheduleFreeState`).
...
Checkpoint functional chain state with :mod:`opake.serialization`
...
``ScheduleFreeState``) live in :mod:`opake.optimizers.types`.
"""

from opake.api.optimizers._adadelta import adadelta
from opake.api.optimizers._adafactor import adafactor
from opake.api.optimizers._adagrad import adagrad
from opake.api.optimizers._adam import adam, adamw
from opake.api.optimizers._ademamix import ademamix
from opake.api.optimizers._lion import lion
from opake.api.optimizers._radam import radam
from opake.api.optimizers._rmsprop import rmsprop
from opake.api.optimizers._schedule_free import schedule_free
from opake.api.optimizers._sgd import sgd
